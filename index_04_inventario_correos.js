...
function requiresClavePlatform(plataforma = "") {
  return !isSoloCorreoPlatform(plataforma);
}

function getPinFromAny(data = {}) {
  const keys = [
    "pin", "PIN", "pinPerfil", "pin_perfil", "perfilPin", "perfil_pin",
    "profilePin", "profile_pin", "pinProfile", "pin_profile",
    "pinCliente", "pin_cliente", "pinCuenta", "pin_cuenta",
  ];
  for (const k of keys) {
    const v = data && data[k];
...
async function enviarInventarioPlataforma(chatId, plataforma = "", page = 0) {
  ...
  try {
    ...
    let txt = `*${escMD(platLabel)}*\n\nDisponibles: ${disponibles.length}\nLlenas: ${llenas.length}`;
    ...
    return upsertPanel(
      ...
      [
        ...
        [{ text: "➕ Nueva cuenta", callback_data: `inv:new:plat:${plat}` }],
        [
...
async function enviarInventarioPlataformaEstado(chatId, plataforma = "", filtro = "disponibles", page = 0) {
  ...
  try {
    ...
    if (!rows.length) {
      return upsertPanel(
        chatId,
        `*${escMD(humanPlatSafe(plat).toUpperCase())}*\n\nNo hay cuentas en la seccion ${filtroNorm === "llenas" ? "llenas" : "disponibles"}.`,
        [
          [
    ...
    txt += `*${tituloEstado}*\n`;
    txt += `Pagina *${safePage + 1}/${totalPages}*\n\n`;
  ...
  try {
    ...
    if (!keys.length) {
      ...
      for (const k of keys) {
        ...
        txt += `   Perfiles: Ocupados: ${x.ocupados}/${x.capacidad}\n`;
        txt += `   OK Disponibles: ${x.disponibles}\n\n`;
      }
    }
...
async function mostrarPanelCorreo(chatId, plataforma = "", acceso = "") {
  ...
  if (requiresClavePlatform(plat)) txt += `Clave: ${escMD(clave || "Sin clave")}\n`;
  if (requiresPinPlatform(plat) && pin) txt += `PIN: ${escMD(pin)}\n`;
  txt += `Perfiles: ${ocupados}/${capacidad} · OK ${disponibles}\n`;
  txt += `${escMD(estado)}`;

  if (isNetflixPlatform(plat) && boolHasCodes(data)) {
    txt += `\n\nNota: Tiene datos/códigos de Netflix disponibles.`;
  }
...
async function responderMenuCodigosNetflix(chatId, plataforma = "", acceso = "") {
  ...
  if (requiresPinPlatform(plat)) {
    ...
    kb.push([{ text: "⏱️ Código temporal", callback_data: `nf_code|temporal|${encodeURIComponent(ident)}` }]);
  }
...
async function responderCodigoNetflix(chatId, acceso = "", tipo = "") {
  ...
  const maps = {
    ...
    pin: [
      ["PIN", getPinFromAny(data) || data.pin_hogar || data.pin_temporal || "No disponible"],
      ["Perfil", data.perfil || data.profile || "No disponible"],
    ],
  };

  const rows = maps[tipoNorm] || [["Dato", "No disponible"]];
  let txt = `Clave: *NETFLIX — ${escMD(tipoNorm.toUpperCase())}*\n\n`;
