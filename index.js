...
/**
 * âœ… SUBLICUENTAS TG BOT â€” INDEX FINAL
 * MENU: Inventario | Clientes | Pagos | Renovaciones | Buscar
 * INVENTARIO: listado por plataforma con paginaciÃ³n (10) + claves visibles
 ...
 * /cliente TELEFONO  -> muestra ficha + botones (RenovÃ³ / Eliminar)
 * /delsub TELEFONO PLATAFORMA CORREO -> elimina un servicio por comando
 * RENOVACIONES: /renovaciones ...   TXT: /txt ...
 * BOTON âœ… RENOVÃ“: registra pago + mueve fecha +30 dÃ­as
...
async function menuPrincipal(chatId) {
  return bot.sendMessage(chatId, "ðŸ“Œ *MENU PRINCIPAL*", {
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: [
        [{ text: "ðŸ“¦ Inventario", callback_data: "menu:inventario" }],
        [{ text: "ðŸ‘¥ Clientes", callback_data: "menu:clientes" }],
        [{ text: "ðŸ’³ Pagos", callback_data: "menu:pagos" }],
        [{ text: "ðŸ“… Renovaciones", callback_data: "menu:renovaciones" }],
        [{ text: "ðŸ”Ž Buscar", callback_data: "menu:buscar" }],
      ],
    },
  });
}
...
async function menuInventario(chatId) {
  return bot.sendMessage(chatId, "ðŸ“¦ *INVENTARIO* (elige plataforma)", {
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: [
        [
          { text: "ðŸ“º Netflix", callback_data: "inv:netflix:0" },
          { text: "ðŸŽ¬ Disney Premium", callback_data: "inv:disneyp:0" },
        ],
        [
          { text: "ðŸŽžï¸ Disney Standard", callback_data: "inv:disneys:0" },
          { text: "ðŸ¿ HBO Max", callback_data: "inv:hbomax:0" },
        ],
        [
          { text: "ðŸŽ¥ Prime Video", callback_data: "inv:primevideo:0" },
          { text: "ðŸ“€ Paramount+", callback_data: "inv:paramount:0" },
        ],
        [{ text: "ðŸ¥ Crunchyroll", callback_data: "inv:crunchyroll:0" }],
        [{ text: "ðŸ“¦ Stock General", callback_data: "inv:general" }],
        [{ text: "â¬…ï¸ Inicio", callback_data: "go:inicio" }],
      ],
    },
  });
}
...
async function menuClientes(chatId) {
  return bot.sendMessage(
    chatId,
    "ðŸ‘¥ *CLIENTES*\n\n" +
      "âž• Nuevo cliente (wizard multi-plataforma)\n" +
      "ðŸ“Œ Ver cliente: /cliente TELEFONO\n" +
      "ðŸ—‘ Eliminar servicio: /delsub TELEFONO plataforma correo\n" +
      "ðŸ“„ Reporte: /clientes_txt\n",
    {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
          [{ text: "âž• Nuevo cliente", callback_data: "cli:nuevo" }],
          [{ text: "ðŸ”Ž Ver cliente", callback_data: "cli:buscar" }],
          [{ text: "â¬…ï¸ Inicio", callback_data: "go:inicio" }],
        ],
      },
    }
...
async function menuPagos(chatId) {
  return bot.sendMessage(
    chatId,
    "ðŸ’³ *PAGOS*\n\n" +
      "Por ahora se registran automaticamente con el boton âœ… RENOVÃ“.\n",
    {
      parse_mode: "Markdown",
      reply_markup: { inline_keyboard: [[{ text: "â¬…ï¸ Inicio", callback_data: "go:inicio" }]] },
    }
  );
}
...
async function menuRenovaciones(chatId) {
  return bot.sendMessage(
      ...
      "â€¢ /txt NOMBRE dd/mm/yyyy\n",
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "ðŸ“… Renovaciones hoy", callback_data: "ren:hoy" }],
          [{ text: "ðŸ“„ TXT hoy", callback_data: "txt:hoy" }],
          [{ text: "â¬…ï¸ Inicio", callback_data: "go:inicio" }],
        ],
      },
    }
...
async function inventarioPlataformaTexto(plataforma, page) {
  ...
  if (slice.length === 0) {
    ...
    for (const d of slice) {
      ...
      texto += `${i}) ${d.correo} â€” ðŸ”‘ ${clave} â€” ${d.disp}/${total ?? "-"}\n`;
      i++;
    }

    texto += `\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”\n`;
    texto += `ðŸ“Š Cuentas con stock: ${totalItems}\n`;
    texto += `ðŸ‘¤ Perfiles libres totales: ${libresTotal}\n`;
  }

  texto += `\nðŸ“„ Pagina: ${safePage + 1}/${totalPages}`;
  return { texto, safePage, totalPages };
}

async function enviarInventarioPlataforma(chatId, plataforma, page) {
  const p = normalizarPlataforma(plataforma);
  if (!esPlataformaValida(p)) return bot.sendMessage(chatId, "âš ï¸ Plataforma invalida.");
  ...
  return bot.sendMessage(chatId, texto, {
    ...
    reply_markup: {
      inline_keyboard: [
        [
          { text: "â¬…ï¸ Atras", callback_data: canBack ? `inv:${p}:${safePage - 1}` : "noop" },
          { text: "ðŸ  Inicio", callback_data: "go:inicio" },
          { text: "âž¡ï¸ Siguiente", callback_data: canNext ? `inv:${p}:${safePage + 1}` : "noop" },
        ],
        [{ text: "ðŸ”„ Actualizar", callback_data: `inv:${p}:${safePage}` }],
...
bot.onText(/\/add\s+(.+)/i, async (msg, match) => {
  ...
  if (!correo.includes("@")) return bot.sendMessage(chatId, "âš ï¸ Correo invalido.");
  if (!esPlataformaValida(plataforma)) return bot.sendMessage(chatId, "âš ï¸ Plataforma invalida.");
  if (!Number.isFinite(disp) || disp < 0) return bot.sendMessage(chatId, "âš ï¸ disp invalido.");
  ...
  return bot.sendMessage(
    chatId,
    `âœ… *Agregada*\nðŸ“Œ ${plataforma.toUpperCase()}\nðŸ“§ ${correo}\nðŸ”‘ ${claveOut}\nðŸ‘¤ Disponibles: ${disp}/${total ?? "-"}\nEstado: *${fmtEstado(data.estado)}*`,
    { parse_mode: "Markdown" }
  );
});
...
bot.onText(/\/del\s+(\S+)\s+(\S+)/i, async (msg, match) => {
  ...
  await ref.delete();
  return bot.sendMessage(chatId, `ðŸ—‘ï¸ Eliminada: ${plataforma.toUpperCase()} â€” ${correo}`);
});
...
bot.onText(/\/addp\s+(.+)/i, async (msg, match) => {
  ...
  if (!Number.isFinite(qty) || qty <= 0) return bot.sendMessage(chatId, "âš ï¸ Cantidad invalida.");
  if (plataforma && !esPlataformaValida(plataforma)) return bot.sendMessage(chatId, "âš ï¸ Plataforma invalida.");
  ...
  if (plataforma) {
    ...
    if (snap.size > 1) return bot.sendMessage(chatId, "âš ï¸ Ese correo estÃ¡ en varias plataformas. Usa: /addp correo plataforma cantidad");
    ref = snap.docs[0].ref;
    d = snap.docs[0].data();
  }
  ...
  return bot.sendMessage(
    chatId,
    `âœ… *Actualizado*\nðŸ“Œ ${String(d.plataforma).toUpperCase()}\nðŸ“§ ${correo}\nðŸ‘¤ Disponibles: ${nuevoDisp}/${total ?? "-"}\nEstado: *${fmtEstado(estadoFinal)}*`,
    { parse_mode: "Markdown" }
  );
});
...
bot.onText(/\/delp\s+(.+)/i, async (msg, match) => {
  ...
  if (plataforma) {
    ...
    const snap = await db.collection("inventario").where("correo", "==", correo).limit(2).get();
    if (snap.empty) return bot.sendMessage(chatId, "âš ï¸ No encontrado.");
    if (snap.size > 1) return bot.sendMessage(chatId, "âš ï¸ Ese correo estÃ¡ en varias plataformas. Usa: /delp correo plataforma cantidad");
    ref = snap.docs[0].ref;
    d = snap.docs[0].data();
  }
...
bot.onText(/\/buscar\s+(.+)/i, async (msg, match) => {
  ...
  let texto = `ðŸ”Ž *BUSQUEDA GENERAL*\nConsulta: \`${q}\`\n\n`;

  if (invSnap.empty && clientes.length === 0) {
    texto += "âš ï¸ Sin resultados.";
    return bot.sendMessage(chatId, texto, { parse_mode: "Markdown" });
  }
  ...
  if (clientes.length > 0) {
    ...
    texto += `\nðŸ“Œ Ver ficha: /cliente TELEFONO`;
  }
...
// ===============================
// CLIENTES â€” WIZARD MULTI-PLATAFORMA + BOTONES
// ===============================
...
function botonesServiciosCliente(clienteId, suscripciones = []) {
  ...
  suscripciones.forEach((s, i) => {
    rows.push([
      { text: `âœ… RenovÃ³ ${String(s.plataforma).toUpperCase()}`, callback_data: `cli_ren:${clienteId}:${i}` },
      { text: `ðŸ—‘ Eliminar ${String(s.plataforma).toUpperCase()}`, callback_data: `cli_del:${clienteId}:${i}` },
    ]);
  });
  rows.push([{ text: "â¬…ï¸ Volver", callback_data: "menu:clientes" }]);
...
async function wizardStart(chatId) {
  ...
  return bot.sendMessage(chatId, "ðŸ‘¥ NUEVO CLIENTE\n(1/3) Escribe: *Nombre perfil*", { parse_mode: "Markdown" });
}
...
async function wizardAskAddMore(chatId) {
  ...
  return bot.sendMessage(chatId, "âœ… Servicio agregado.\nÂ¿Deseas agregar *otra plataforma* a este cliente?", {
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: [
        [{ text: "âž• Agregar otra", callback_data: "wiz:addmore" }],
        [{ text: "âœ… Finalizar", callback_data: "wiz:finish" }],
      ],
    },
  });
}
...
async function wizardFinalizar(chatId) {
  ...
  if (!st.suscripciones || st.suscripciones.length === 0) {
    wizardReset(chatId);
    return bot.sendMessage(chatId, "âš ï¸ No se agrego ningun servicio. Cancelado.");
  }
  ...
  let t = `âœ… Cliente agregado\n\n`;
  t += `ðŸ“Œ Datos del cliente\n`;
  t += `Nombre: ${payload.nombrePerfil}\n`;
  t += `Telefono: ${payload.telefono}\n`;
  t += `Vendedor: ${payload.vendedor}\n\n`;
  t += `ðŸ“º Servicios\n`;
  payload.suscripciones.forEach((s, i) => {
    t += `${i + 1}) ${s.plataforma} â€” ${s.correo} â€” ${s.precio} Lps â€” Renueva: ${s.fechaRenovacion}\n`;
  });

  t += `\nðŸ“Œ Ver ficha: /cliente ${payload.telefono}`;
  ...
  if (st.step === 4) {
    ...
    switch (st.subStep) {
      case 1: {
        ...
        if (!esPlataformaValida(p)) return bot.sendMessage(chatId, "âš ï¸ Plataforma invalida. Intenta de nuevo:");
        sub.plataforma = p;
        st.currentSub = sub;
        st.subStep = 2;
      ...
      case 2:
        if (!t.includes("@")) return bot.sendMessage(chatId, "âš ï¸ Correo invalido. Intenta de nuevo:");
      ...
      case 4: {
        const n = Number(t);
        if (!Number.isFinite(n) || n <= 0) return bot.sendMessage(chatId, "âš ï¸ Precio invalido. Escribe solo numero:");
        sub.precio = n;
        st.currentSub = sub;
        st.subStep = 5;
      ...
      case 5:
        if (!isFechaDMY(t)) return bot.sendMessage(chatId, "âš ï¸ Formato invalido. Usa dd/mm/yyyy:");
...
bot.onText(/\/cliente\s+(\S+)/i, async (msg, match) => {
  ...
  if (snap.empty) return bot.sendMessage(chatId, "âš ï¸ Cliente no encontrado por telefono.");

  const doc = snap.docs[0];
  const c = doc.data() || {};
  const sus = Array.isArray(c.suscripciones) ? c.suscripciones : [];

  let texto = `ðŸ‘¤ *CLIENTE*\n\n`;
  texto += `ðŸ“Œ Datos del cliente\n`;
  texto += `Nombre: ${c.nombrePerfil || "-"}\n`;
  texto += `Telefono: ${c.telefono || "-"}\n`;
  texto += `Vendedor: ${c.vendedor || "-"}\n\n`;

  texto += `ðŸ“º *SERVICIOS*\n`;
  if (sus.length === 0) texto += "âš ï¸ Sin servicios.\n";
  else {
    sus.forEach((s, i) => {
      texto += `${i + 1}) ${s.plataforma} â€” ${s.correo} â€” ${s.precio} Lps â€” Renueva: ${s.fechaRenovacion}\n`;
    });
  }
...
bot.onText(/\/delsub\s+(\S+)\s+(\S+)\s+(\S+)/i, async (msg, match) => {
  ...
  const snap = await db.collection("clientes").where("telefono", "==", telefono).limit(1).get();
  if (snap.empty) return bot.sendMessage(chatId, "âš ï¸ Cliente no encontrado.");
  ...
  if (sus.length === antes) return bot.sendMessage(chatId, "âš ï¸ Servicio no encontrado.");

  await ref.set({ suscripciones: sus, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });

  return bot.sendMessage(chatId, `ðŸ—‘ï¸ Servicio eliminado\nðŸ“± ${telefono}\nðŸ“Œ ${plataforma.toUpperCase()}\nðŸ“§ ${correo}`);
});
...
bot.on("callback_query", async (q) => {
  ...
  try {
    ...
    if (data === "menu:buscar") return bot.sendMessage(chatId, "ðŸ”Ž Usa: /buscar texto");
    ...
    if (data === "cli:buscar") return bot.sendMessage(chatId, "ðŸ”Ž Ver ficha: /cliente TELEFONO");
    ...
    if (data.startsWith("cli_del:")) {
      ...
      const doc = await ref.get();
      if (!doc.exists) return bot.sendMessage(chatId, "âš ï¸ Cliente no existe.");

      const cliente = doc.data() || {};
      const sus = Array.isArray(cliente.suscripciones) ? cliente.suscripciones : [];
      if (!sus[index]) return bot.sendMessage(chatId, "âš ï¸ Servicio no existe.");

      const s = sus[index];
      return bot.sendMessage(
        chatId,
        `âš ï¸ Confirmar eliminar\n\n${cliente.nombrePerfil || "-"}\n${cliente.telefono || "-"}\n\n${s.plataforma}\n${s.correo}`,
        {
          reply_markup: {
            inline_keyboard: [
              [
                { text: "âœ… Confirmar", callback_data: `cli_del_ok:${clienteId}:${index}` },
                { text: "âŒ Cancelar", callback_data: "noop" },
              ],
            ],
          },
        }
    ...
    if (data.startsWith("cli_del_ok:")) {
      ...
      return bot.sendMessage(chatId, `ðŸ—‘ Servicio eliminado\n${eliminado.plataforma} â€” ${eliminado.correo}\n\nðŸ“Œ Ver: /cliente ${cliente.telefono}`);
    }
    ...
    if (data.startsWith("cli_ren:")) {
      ...
      return bot.sendMessage(
        chatId,
        `âš ï¸ Confirmar RENOVACION\n\n${cliente.nombrePerfil || "-"}\n${cliente.telefono || "-"}\n\nServicio: ${s.plataforma}\nCorreo: ${s.correo}\nPrecio: ${s.precio} Lps\n\nFecha actual: ${s.fechaRenovacion}\nNueva fecha: ${nuevaFecha}`,
        {
          reply_markup: {
            inline_keyboard: [
              [
                { text: "âœ… Confirmar", callback_data: `cli_ren_ok:${clienteId}:${index}` },
                { text: "âŒ Cancelar", callback_data: "noop" },
              ],
            ],
          },
    ...
    if (data.startsWith("cli_ren_ok:")) {
      ...
      return bot.sendMessage(chatId, `âœ… Renovacion registrada\nNueva fecha: ${fechaNueva}\nPago guardado.\n\nðŸ“Œ Ver: /cliente ${cliente.telefono}`);
    }

    return bot.sendMessage(chatId, "âš ï¸ Accion no reconocida.");
  } catch (err) {
    console.log("âŒ callback_query error:", err?.message || err);
