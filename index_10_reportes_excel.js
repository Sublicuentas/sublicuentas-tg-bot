/* ✅ SUBLICUENTAS — REPORTES EXCEL PROFESIONAL (NIVEL DIOS)
   100% COMPATIBLE — Solo métodos seguros de ExcelJS
   - Barras visuales con bloques (texto puro, nunca falla)
   - Encabezados congelados (ws.views)
   - Filtros automáticos
...
function logErr(scope = "error", err = "") {
  try { console.error(`❌ [${scope}]`, err && err.message ? err.message : err); } catch (_) {}
}

const C = {
  rojo: "FFD32F2F", rojoOsc: "FFB71C1C", negro: "FF1A1A1A",
  verde: "FF2E7D32", blanco: "FFFFFFFF", gris: "FFF5F5F5",
  grisOsc: "FF424242", dorado: "FFFFB300", azul: "FF1565C0",
};
...
function barra(valor, max, largo) {
  const m = Number(max) || 1;
  const pct = Math.round((Math.abs(Number(valor) || 0) / m) * largo);
  return "\u2588".repeat(Math.max(1, pct));
}
...
async function generarReporteExcelPorRango(fechaInicio, fechaFin) {
  try {
    ...
    ws1.columns = [{ width: 4 }, { width: 30 }, { width: 22 }, { width: 28 }];
    ws1.addRow([]);
    ...
    const kpis = [
      ["💰 Total Ingresos", totIng, C.verde, '"Lps " #,##0.00'],
      ["💸 Total Egresos", totEgr, C.rojo, '"Lps " #,##0.00'],
      ["📈 Utilidad Neta", utilidad, utilidad >= 0 ? C.verde : C.rojoOsc, '"Lps " #,##0.00'],
      ["📊 Margen", margen / 100, C.azul, '0.00%'],
      ["🧾 Movimientos", movimientos.length, C.grisOsc, '#,##0'],
    ];
    kpis.forEach(([label, valor, color, fmt]) => {
    ...
    cmpHead.eachCell((c, n) => {
      if (n > 1) {
        c.font = { bold: true, color: { argb: C.blanco } };
        c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: C.negro } };
        c.border = BORDE;
    ...
    [["Ingresos", totIng, C.verde], ["Egresos", totEgr, C.rojo]].forEach(([lbl, val, col]) => {
      const r = ws1.addRow(["", lbl, val, barra(val, maxVal, 20)]);
      ...
      r.getCell(4).font = { color: { argb: col }, size: 11 }; r.getCell(4).border = BORDE;
    });
    ...
    if (ingresos.length > 0) ws2.autoFilter = { from: { row: ing0, column: 1 }, to: { row: ing1, column: 6 } };
    ...
    if (egresos.length > 0) ws3.autoFilter = { from: { row: egr0, column: 1 }, to: { row: egr1, column: 6 } };
    ...
    ws4.columns = [{ width: 22 }, { width: 18 }, { width: 18 }, { width: 18 }, { width: 18 }];
    ...
    let maxNeto = 1;
    entries.forEach(([, d]) => { const n = Math.abs(d.ing - d.egr); if (n > maxNeto) maxNeto = n; });
    entries.forEach(([banco, d], i) => {
      const neto = d.ing - d.egr;
      const r = ws4.addRow([banco, d.ing, d.egr, neto, barra(neto, maxNeto, 15)]);
      r.eachCell(c => { c.border = BORDE; });
      if (i % 2 === 1) r.eachCell(c => { c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: C.gris } }; });
    ...
    ws5.columns = [{ width: 8 }, { width: 26 }, { width: 18 }, { width: 14 }, { width: 22 }];
    ...
    let maxPlat = 1;
    topPlats.forEach(([, d]) => { if (d.monto > maxPlat) maxPlat = d.monto; });
    topPlats.forEach(([plat, d], i) => {
      const medalla = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i + 1}`;
      const r = ws5.addRow([medalla, plat, d.monto, d.count, barra(d.monto, maxPlat, 18)]);
      r.eachCell(c => { c.border = BORDE; });
      if (i % 2 === 1) r.eachCell(c => { c.fill = { type: "pattern", pattern: "solid", fgColor: { argb: C.gris } }; });
      r.getCell(1).alignment = { horizontal: "center" };
      r.getCell(3).numFmt = '"Lps " #,##0.00';
